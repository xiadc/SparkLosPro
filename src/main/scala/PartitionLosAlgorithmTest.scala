...
/**
  * Created by xiadc on 2018-04-23.
  */
object PartitionLosAlgorithmTest {
  ...
  def main(args: Array[String]): Unit = {
   ...
   /* val conf = new SparkConf().setAppName("LOS_Spark_Algorithm").setMaster("spark://master:7077")*/
    ...
    //设置spark最大分区字节数为1G
    conf.set("spark.files.maxPartitionBytes","1024")
    ...
    def doLOS(keyValue: (Int, mutable.HashMap[Long,Float])) = {
      ...
      val resultFloatHashMap = new mutable.HashMap[Long,Array[Byte]]()
      //遍历该可变集合hashmap，对每个键值对处理
      for ((currentPos,elevation) <- hashmap) {
        var tempByteArray = new Array[Byte](4)
        if(doubleEquals(elevation,InvalidValue)) {//高程值无效值，直接视为可视
          ...
          val flag = isVisible(px, py, realElevation,hashmap)
          tempByteArray = if (flag) getFloatByte(0.0f) else getFloatByte(1.0f)
         // println("eyePnt:"+eyePnt.x+" "+eyePnt.y+" "+eyePnt.z+" "+currentPos+" "+elevation+" "+flag)
        }
        resultFloatHashMap.put(currentPos,tempByteArray)

      }
      //返回键为1，值为TreeMap的键值对，TreeMap按键排序
      (1L,resultFloatHashMap)
    }
    //字节数组转float数组
    ...
    def getPartitionStartCoordinate(nx:Int,ny:Int,everyGridNum:Double,rank:Int) ={
      ...
      if(rank != 0) { //第一个分块的起点设置为左下角点（0,0）
        ...
        val twoGridNum = (ny * (nx - rasEyePnt.rasX )).toDouble / 2 + oneGridNum
        val threeGridNum = (nx * (ny - rasEyePnt.rasY)).toDouble / 2 + twoGridNum
    ...
    def transferHashMapToByteArray(keyValue:(Long,mutable.HashMap[Long,Array[Byte]])) ={
      val hashmap = keyValue._2
      val resultByteArray = new Array[Byte](hashmap.size*4)
      //将hashmap中的数据转换为字节数组
      for(i <- 0 to hashmap.size-1){
        resultByteArray(i*4) = hashmap.get(i).get(0)
        resultByteArray(i*4+1) = hashmap.get(i).get(1)
        resultByteArray(i*4+2) = hashmap.get(i).get(2)
        resultByteArray(i*4+3) = hashmap.get(i).get(3)
      }
      //键值对序列化
    ...
    val a = gridRdd.zipWithIndex().map(keyValue=>(keyValue._2 - 15,keyValue._1)) //转换key为0开始的键值对，
